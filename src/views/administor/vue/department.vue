...
<el-row style="margin-left:20px;">
    ...
    <el-col :span="8" style="text-align: right;padding-right:10px;">
        <el-button-group>
            <el-button type="primary" icon="el-icon-search" @click="querydepartment()">筛选</el-button>
            <el-button type="primary" icon="el-icon-tickets" @click="getdepartment()">全部</el-button>
            <el-button type="primary" icon="el-icon-circle-plus-outline" @click="adddepartment()">添加</el-button>
        </el-button-group>
    </el-col>
</el-row>
</el-form>
    <el-table :data="pagedepartment" border style="width: 90%;margin-left:20px" size="mini" @selection-change="handleSelectionChange">
        ...
        <el-table-column width="400px" fixed="right" label="操作" align="center">
            <template slot-scope="scope">
                <el-button type="success" icon="el-icon-more" size="mini" circle @click="viewdepartment(scope.row)"></el-button>
                <el-button type="primary" icon="el-icon-edit" size="mini" circle @click="updatedepartment(scope.row)"></el-button>
                <el-button type="danger" icon="el-icon-delete" size="mini" circle @click="deletedepartment(scope.row)"></el-button>
    ...
    <el-row style="margin-top: 10px;">
        <el-col :span="8" style="text-align: left;margin-left:20px">
            <el-button type="primary" icon="el-icon-delete" size="mini" @click="deletedepartments()">批量删除</el-button>
        </el-col>
      ...
      <el-dialog :title="dialogTitle" :visible.sync="dialogVisible" width="50%"  @close="closeDialogForm('departmentform')">
        <el-form :model="departmentform" :rules="rules" ref="departmentform" :inline=true style="margin-left:20px;" label-width="110px" label-position="right" size="mini">
            <el-form-item label="ID" prop="ID" v-if="adddialog || viewdialog || updatedialog || deletedialog">
                <el-input v-model="departmentform.ID" :disabled="isEdit || isView" suffix-icon="el-icon-edit"></el-input>
            </el-form-item>
            <el-form-item label="备注" prop="Rem" v-if="adddialog || viewdialog || updatedialog || deletedialog">
                <el-input v-model="departmentform.Rem" maxlength="64" :disabled="isView" suffix-icon="el-icon-edit"></el-input>
            </el-form-item>
            <el-form-item label="简介" prop="Introduction" v-if="adddialog || viewdialog || updatedialog || deletedialog">
                <el-input v-model="departmentform.Introduction" maxlength="254" :disabled="isView" suffix-icon="el-icon-edit"></el-input>
            </el-form-item>
            <el-form-item label="最后操作人ID" prop="IdManager" v-if="adddialog || viewdialog || updatedialog || deletedialog">
                <el-input v-model="departmentform.IdManager"  :disabled="isView" suffix-icon="el-icon-edit"></el-input>
            </el-form-item>
            <el-form-item label="更新时间" prop="TimeUpdate" v-if="adddialog || viewdialog || updatedialog || deletedialog">
                <el-input v-model="departmentform.TimeUpdate"  :disabled="isView" suffix-icon="el-icon-edit"></el-input>
            </el-form-item>
            <el-form-item label="存在标记" prop="IMark" v-if="adddialog || viewdialog || updatedialog || deletedialog">
                <el-input v-model="departmentform.IMark"  :disabled="isView" suffix-icon="el-icon-edit"></el-input>
            </el-form-item>
            <el-form-item label="父级部门" prop="ID_Parent" v-if="adddialog || viewdialog || updatedialog || deletedialog">
                <el-input v-model="departmentform.ID_Parent"  :disabled="isView" suffix-icon="el-icon-edit"></el-input>
            </el-form-item>
            <el-form-item label="部门名称" prop="Name" v-if="adddialog || viewdialog || updatedialog || deletedialog">
                <el-input v-model="departmentform.Name"  :disabled="isView" suffix-icon="el-icon-edit"></el-input>
            </el-form-item>
            <el-form-item label="部门简称" prop="Name2" v-if="adddialog || viewdialog || updatedialog || deletedialog">
                <el-input v-model="departmentform.Name2"  :disabled="isView" suffix-icon="el-icon-edit"></el-input>
            </el-form-item>
            <el-form-item label="修改者名字" prop="ID_Manager_Name" v-if="adddialog || viewdialog || updatedialog || deletedialog">
                <el-input v-model="departmentform.ID_Manager_Name"  :disabled="isView" suffix-icon="el-icon-edit"></el-input>
            </el-form-item>
        </el-form>
        <span slot="footer" class="dialog-footer">
            <el-button type="primary" size="mini" v-show="!isView" @click="submitdepartmentform('departmentform')">确定</el-button>
            <el-button type="info" size="mini" @click="closeDialogForm('departmentform')">取消</el-button>
        </span>
      </el-dialog>
</div>
</template>
...
export default {
    ...
    mounted(){
     ...
     methods:{
         timeup1(){
            this.adddepartmentform.data[0].TimeUpdate = new Date().getTime() - new Date
            ('2000-1-1 00:00:00').getTime();
        },
        timeup2(){
            this.updatedepartmentform.data[0].TimeUpdate = new Date().getTime() - new Date
            ('2000-1-1 00:00:00').getTime();
        },
        //详情生成该部门签到记录表
        handleclick(row){
...
// })},500);
        ...
        getpagedepartment(){
            ...
            for(let i=(this.currentpage -1)*this.pagesize; i< this.total;i++){
                ...
                //判断是否达到一页的要求
                if (this.pagedepartment.length === this.pagesize) break;
            }
        },
        //实现当前页的设备信息筛选
        querydepartment(){
        ...
        submitadddepartment(){
            ...
            this.adddepartmentform.data[0].ID_Parent =this.departmentform.ID_Parent*1;
            this.adddepartmentform.data[0].Name =this.departmentform.Name;
            this.adddepartmentform.data[0].Name2 =this.departmentform.Name2;
            let that = this;
            //执行Axios请求
            axios
            .post(that.baseURL + 'model_department/', that.adddepartmentform)
            .then(res=>{
                //执行成功
                if(true){//res.data.code == 1
                    // //获取所有部门信息
        ...
        submitupdatadepartment(){
            ...
            this.updatedepartmentform.data[0].ID_Parent =this.departmentform.ID_Parent*1;
            ...
            //执行Axios请求
            axios
            .put(that.baseURL + 'model_department/', that.updatedepartmentform)
            .then(res=>{
                //执行成功
                if(true){//res.data.code == 1
                    // //获取所有部门信息
        ...
        deletedepartment(row) {
            //等待确认
            this.$confirm('是否确认删除部门信息【部门ID：' + row.ID + '\t部门名称：' + row.name + '】信息？',
                '提示', {
                ...
                //调用后端接口
                axios
                .delete(that.baseURL + 'model_department/',{data:{ids:[{"data_id":row.ID}],n:1}})
                .then(res => {
                        if (true) {
                            // //获取所有部门信息
                            // that.department = res.data.data;
        ...
        deletedepartments() {
            //等待确认
            this.$confirm("是否确认批量删除" + this.selectdepartments.length + "个部门信息吗？",
                '提示', {
                confirmButtonText: '确定删除',
                cancelButtonText: '取消',
                type: 'warning'
                ...
                for(var i=0;i<this.selectdepartments.length;i++){
                    ids.push({"data_id":this.selectdepartments[i].ID*1});
                }
                //确认删除响应事件
                let that = this
                //调用后端接口
                axios
                    .delete(this.baseURL + 'model_department/',{data:{
                    ids:ids},n:this.selectdepartments.length
                    })
                    .then(res => {
                        if (true) {
                            // //获取所有部门信息

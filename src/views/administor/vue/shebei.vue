<template>
  <div>
    ...
    <el-dialog
      ...
      width="50%"
      @close="closeDialogForm('shebeiform')"
    >
      <el-form
        :model="shebeiform"
        :rules="rules"
      ...
      >
        ...
        >
          <el-input
            v-model="shebeiform.Introduction"
            :disabled="isView"
            suffix-icon="el-icon-edit"
          ></el-input>
        </el-form-item>
        ...
        >
          <el-input
            v-model="shebeiform.ID_Location"
            :disabled="isView"
            suffix-icon="el-icon-edit"
          ></el-input>
        </el-form-item>
        ...
        >
          <el-input
            v-model="shebeiform.ID_Location_SN"
            :disabled="isView"
            suffix-icon="el-icon-edit"
          ></el-input>
        </el-form-item>
      ...
      <span slot="footer" class="dialog-footer">
        <el-button
          ...
          size="mini"
          v-show="!isView"
          @click="submitshebeiform('shebeiform')"
          >确定</el-button
        >
        <el-button
          type="info"
          size="mini"
          @click="closeDialogForm('shebeiform')"
          >取消</el-button
        >
      </span>
    </el-dialog>
...
export default {
  data() {
    ...
    const rulesID = (rule, value, callback) => {
      ...
      //使用axios进行校验
      axios
        .post(this.baseURL + "model_equipment/", {
          id: value,
          kind: 2
        })
        .then(res => {
    ...
    return {
      ...
      rules: {
        ...
        iTimeBegin: [{ required: true, message: "为空填0", trigger: "blur" }],
        iTimeLogin: [{ required: true, message: "为空填0", trigger: "blur" }],
        Rem: [{ required: true, message: "设备说明为空填0", trigger: "blur" }],
        PortListen: [
  ...
  methods: {
    ...
    timeup() {
      this.shebeiform.Timeupdate =
        (new Date().getTime() - new Date("2000-1-1 00:00:00").getTime()) / 1000;
    },
    ...
    getshebei: function() {
      ...
      //使用Axios实现Ajax请求
      //console.log(localStorage.getItem("token_type")+' '+localStorage.setItem("token"))
      axios
        ({url:"/api/" + "model_equipment/search",
        method:'post',
        headers:{
    ...
    queryshebei() {
      ...
      //开始Ajax请求
      axios(
        {url:"/api/" + "model_equipment/search?page=1&size=5", 
        method:'post',
        headers:{
        'accept': "application/json",
        'Authorization':'Bearer'+" "+this.webtoken
    ...
    submitaddshebei() {
      ...
      this.addshebeiform.data[0].ID_IP = this.shebeiform.ID_IP;
      ...
      this.addshebeiform.data[0].Type = this.shebeiform.Type * 1;
      ...
      //执行Axios请求
      axios
        ({url:"/api/" + "model_equipment/", 
        method:'post',
        headers:{
        'accept': "application/json",
        'Authorization':'Bearer'+" "+this.webtoken
      },
        data:that.addshebeiform})
        .then(res => {
          //执行成功
          if (true) {
            //res.data.code == 1
    ...
    submitupdatashebei() {
      ...
      this.updateshebeiform.data[0].ID_IP = this.shebeiform.ID_IP;
      ...
      this.updateshebeiform.data[0].Type = this.shebeiform.Type * 1;
    ...
    deleteshebeis() {
      //等待确认
      this.$confirm(
        "是否确认批量删除" + this.selectshebeis.length + "个设备信息吗？",
        "提示",
        {
          confirmButtonText: "确定删除",
          cancelButtonText: "取消",
        ...
        .then(() => {
          ...
          for (var i = 0; i < this.selectshebeis.length; i++) {
            data.push({ ID: this.selectshebeis[i].ID*1 });
          }
          //确认删除响应事件
          let that = this;
          //调用后端接口

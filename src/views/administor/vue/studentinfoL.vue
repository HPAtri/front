      ...
      >
        ...
        >
          <el-date-picker
            type="date"
            placeholder="选择日期"
            v-model="userform.userex_related_to_user_information__timeupdate"
            value-format="yyyy-MM-dd"
            :disabled="isView"
            suffix-icon="el-icon-edit"
          >
        ...
        <el-form-item
          label="管理员姓名"
          prop="userex_related_to_user_information__idmanager__name"
          v-if="adddialog || viewdialog || updatedialog || deletedialog"
        >
          <el-input
            v-model="
        ...
        <el-form-item
          label="描述"
          prop="userex_related_to_user_information_rem"
          v-if="adddialog || viewdialog || updatedialog || deletedialog"
        >
          <el-input
            v-model="userform.userex_related_to_user_information_rem"
            :disabled="isView"
            suffix-icon="el-icon-edit"
          ></el-input>
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    submitadduser() {
      ...
      //执行Axios请求
      axios
        .put("/api/" + "StudentsInformation/", that.userform)
        .then(res => {
          //执行成功
          if (true) {
            //res.data.code == 1
    ...
    submitupdatauser() {
      ...
      //执行Axios请求
      axios
        .patch("/api/" + "StudentsInformation/", that.userform)
        .then(res => {
          //执行成功
          if (true) {
            //res.data.code == 1
    ...
    deleteuser(row) {
      ...
      this.$confirm(
        ...
        .then(() => {
          ...
          //调用后端接口
          axios
            .delete("/api/" + "StudentsInformation/", {
              data: { ids: [{ data_id: row.id }] }
            })
            .then(res => {
              if (true) {
                that.getuser();

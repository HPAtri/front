...
<el-row style="margin-left:20px;">
    ...
    <el-table :data="pageuser" border style="width: 90%;margin-left:20px" size="mini" @selection-change="handleSelectionChange">
        ...
        <el-table-column prop="name" label="使用者的姓名" width="140" align="center">
        </el-table-column>
        <el-table-column prop="sex" label="性别" width="140" align="center">
        </el-table-column>
        <el-table-column prop="deptid__name" label="部门" width="140" align="center">
        </el-table-column>
    ...
    <el-dialog :title="dialogTitle" :visible.sync="dialogVisible" width="50%"  @close="closeDialogForm('userform')">
        <el-form :model="userform" :rules="rules" ref="userform" :inline=true style="margin-left:20px;" label-width="110px" labei-position="right" size="mini">
            ...
            <el-form-item label="使用者的姓名" prop="name" v-if="adddialog || viewdialog || updatedialog || deletedialog">
                <el-input v-model="userform.name" :disabled="isView" suffix-icon="el-icon-edit"></el-input>
            </el-form-item>
            <el-form-item label="使用者的密码" prop="psw" v-if="adddialog || viewdialog || updatedialog || deletedialog">
            ...
            <el-form-item label="性别" prop="sex" v-if="adddialog || viewdialog || updatedialog || deletedialog">
                <el-select v-model="userform.sex" :disabled="isView" placeholder="请选择性别">
                    <el-option labei="女性" value="女性"></el-option>
                    <el-option labei="男性" value="男性"></el-option>
                </el-select>
            </el-form-item>
            <el-form-item label="权限" prop="attr" v-if="adddialog || viewdialog || updatedialog || deletedialog">
                <el-select v-model="userform.attr" :disabled="isView" placeholder="请选择身份">
                    <el-option labei="超级管理员" value="超级管理员"></el-option>
                    <el-option labei="教务处管理员" value="教务处管理员"></el-option>
                    <el-option labei="辅导员" value="辅导员"></el-option>
                    <el-option labei="教师" value="教师"></el-option>
                    <el-option labei="学生" value="学生"></el-option>
                </el-select>
            </el-form-item>
            <el-form-item label="信息更新时间" v-if="adddialog || viewdialog || updatedialog || deletedialog">
                <el-date-picker type="date" placeholder="选择日期" v-model="userform.timeupdate" value-format="yyyy-MM-dd" :disabled="isView" suffix-icon="el-icon-edit">
                </el-date-picker>
            </el-form-item>
            ...
            <el-form-item label="timeupdate" v-if="adddialog || viewdialog || updatedialog || deletedialog">
                <el-date-picker type="date" placeholder="选择日期" v-model="userform.userex_related_to_user_information__timeupdate" value-format="yyyy-MM-dd" :disabled="isView" suffix-icon="el-icon-edit">
                </el-date-picker>
            </el-form-item>
            ...
            <el-form-item label="描述" prop="userex_related_to_user_information_rem" v-if="adddialog || viewdialog || updatedialog || deletedialog">
                <el-input v-model="userform.userex_related_to_user_information_rem" :disabled="isView" suffix-icon="el-icon-edit"></el-input>
            </el-form-item>
        </el-form>
...
export default {
    data(){
        ...
        userform:{
            ...
            userex_related_to_user_information_rem:'',
        },
        rules:{
            id:[
    ...
    mounted(){
     ...
     methods:{
        ...
        getuser:function(){
            ...
            //使用Axios实现Ajax请求
            axios
            .get(that.baseURL + 'StudentsInformation/')
            .then(function(res){
                //请求成功后执行的函数
                console.log(res);
                if(res.data.code == 0){
        ...
        queryuser(){
            ...
            axios                                                                                                                 //Axios请求
            .post(
                that.baseURL + "StudentsInformation/",
                {
        ...
        closeDialogForm(formName){
            ...
            this.userform.userex_related_to_user_information_rem ="";
        ...
        submitadduser(){
            ...
            //执行Axios请求
            axios
            .put(that.baseURL + 'StudentsInformation/', that.userform)
            .then(res=>{
                //执行成功
                if(true){//res.data.code == 1
                    that.getuser();
        ...
        submitupdatauser(){
            ...
            //执行Axios请求
            axios
            .patch(that.baseURL + 'StudentsInformation/', that.userform)
            .then(res=>{
                //执行成功
                if(true){//res.data.code == 1
                    // //获取所有用户信息
            ...
            .catch(err=>{
                ...
                errcatch(err);;
                //that.$message.error("获取后端结果出现异常！");
                that.$message.error(err.data.error_message);
            });
        },
        //删除一条用户记录
        deleteuser(row) {
            ...
            this.$confirm('是否确认删除用户信息【用户id：' + row.id + '\t姓名：' + row.nocard + '】信息？',
                ...
                //调用后端接口
                axios
                .delete(that.baseURL + 'StudentsInformation/',{data:{ids:[{"data_id":row.id}]}})
                .then(res => {
                        if (true) {
                            that.getuser();
                            // //获取所有用户信息
        ...
        deleteusers() {
            ...
            this.$confirm("是否确认批量删除" + this.selectusers.length + "个用户信息吗？",
                ...
                let that = this
                //调用后端接口
                axios.delete(that.baseURL + 'StudentsInformation/',{data:{
                ids:ids}
                })
                .then(res => {
                        if (true){
